...
function CTimerCtrl.Release(self)
	self.m_TimerDict = {}
	self.m_TimerIDList = {}
	self.m_LateTimerIDList = {}
end

function CTimerCtrl.AddTimer(self, cbfunc, delta, delay, unscaled, lateupdate)
	local iTimerID = Utils.GetUniqueID()
	local iElapsed = unscaled and UnityEngine.Time.unscaledTime or UnityEngine.Time.time
...
function CTimerCtrl.UpdateList(self, list)
	...
	for i, id in ipairs(list) do
		...
		if v then 
			local iElapsed = v.unscaled and iUnscaledTime or iTime
