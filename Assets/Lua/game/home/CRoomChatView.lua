local o = class("CRoomChatView",CViewBase)

o.Config = {
    PrefabPath = "ChinaTown/UI/Home/RoomChatView",
...
function o:AddChatMsg(args)
    local index = g_RoomCtrl:GetMemberIndex(args.pid)
    if not index then
        return
    end
    local pos = {x= -714 + (index * 230),y=280}
